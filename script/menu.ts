...
window.addEventListener('DOMContentLoaded', () => {
  ...
  if(root) root.replaceWith(menuContainer);
  else document.body.appendChild(menuContainer);
  ...
  layoutSelect?.addEventListener('change', e => {
    localStorage.setItem('keyboardLayout', (e.target as HTMLSelectElement).value);
  });
  volumeSlider?.addEventListener('change', e => {
    localStorage.setItem('gameVolume', (e.target as HTMLInputElement).value);
  });
  // Charger préférences
  ...
  playBtn?.addEventListener('click', () => {
    ...
    import('./game.js').then(mod => {
      if(mod && typeof mod.startGame === 'function') mod.startGame();
    });
  });
  ...
  menuBtn?.addEventListener('click', () => {
    ...
    import('./game.js').then(mod => {
      if(mod && typeof mod.stopGame === 'function') mod.stopGame();
    });
  });
});
